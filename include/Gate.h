...
public:
    ...
    // 막혀 있으면 false
    bool tryTeleport(const int newY, const int newX, const Direction inDir, Map &map,
...
private:
    ...
    // 가장자리 wall 인지 검사
    bool isEdge(const Map &map, const int y, const int x) const;

    // 가장자리 Gate 의 진출 방향
    // 맵 안쪽으로 고정
    Direction edgeExitDir(const Map &map, const int y, const int x) const;
    ...
    //  4) 반대방향
    // 다 막히면 DIR_NONE 반환
    Direction interiorExitDir(const Map &map, const int y, const int x, const Direction inDir) const;
};
