...
bool Gate::isEdge(const Map &map, const int y, const int x) const
{
    if (y == 0)
        return true;
...
Direction Gate::edgeExitDir(const Map &map, const int y, const int x) const
{
...
Direction Gate::interiorExitDir(const Map &map, const int y, const int x, const Direction inDir) const
{
    // 우선순위: 직진 -> 시계방향 -> 반시계방향 -> 반대방향
    const Direction cands[4] = {inDir, rotateCW(inDir), rotateCCW(inDir), reverseDir(inDir)};
...
bool Gate::spawnPair(Map &map, const Snake & /*snake*/)
{
    ...
    // 1. 게이트 출구 방향으로 진출이 논리적으로 가능한 벽(WALL) 좌표만 후보군에 적립
    for (int y = 0; y < H; y++)
    {
        for (int x = 0; x < W; x++)
        {
            if (map.getCell(y, x) != WALL)
                continue; // Immune Wall 제외

            bool ok = false;
            if (isEdge(map, y, x))
            {
                // 가장자리: 안쪽 방향 한 칸이 통과 가능해야 함
                const Direction d = edgeExitDir(map, y, x);
                int dy, dx;
                dirToDelta(d, dy, dx);
...
bool Gate::tryTeleport(const int newY, const int newX, const Direction inDir, Map &map,
                       int &outY, int &outX, Direction &outDir)
{
    ...
    // 어느 쪽 Gate에 진입했는지 식별
    int idx = -1;
    if (newY == pos[0].y && newX == pos[0].x)
        idx = 0;
    else if (newY == pos[1].y && newX == pos[1].x)
        idx = 1;
    else
        return false;
