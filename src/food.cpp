// food.cpp
// Food 클래스의 구현 파일.
//   - 성장 아이템(GROWTH_ITEM)의 스폰 위치 선정 및 수명 주기를 연산함
...
// 맵 상의 빈 공간(EMPTY)을 수집하여 임의의 위치에 성장 아이템을 스폰함
void Food::spawn(Map &map)
{
...
void Food::update(Map &map)
{
    ...
    if (map.getCell(y, x) != GROWTH_ITEM)
    {
        spawn(map);
        return;
    }
