...
bool Map::loadFromFile(const char *const filename)
{
...
int Map::getCell(const int y, const int x) const
{
    if (y < 0 || y >= height)
        return WALL;
    if (x < 0 || x >= width)
...
// 한 칸 값 바꾸기
void Map::setCell(const int y, const int x, const int value)
{
    if (y < 0 || y >= height)
        return;
    if (x < 0 || x >= width)
...
void Map::draw(const int offsetY, const int offsetX) const
{
    for (int y = 0; y < height; y++)
    {
        for (int x = 0; x < width; x++)
